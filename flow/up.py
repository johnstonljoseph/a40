...
class _BlendActivationFunction(Function):
    ...
    def backward(
        ...
        slope = torch.ones_like(gate) * 0.5

        dact_dgate = dsilu * (1.0 - blend) + slope * blend
        grad_gate = grad_output * dact_dgate

        grad_a_p = torch.zeros_like(a_p)
        grad_a_m = torch.zeros_like(a_m)
...
def main() -> None:
    ...
    def maybe_update_lr_or_exit() -> tuple[bool, bool]:
        ...
        return new_base_lr is not None, exit_requested
    ...
    try:
        ...
        while True:
            ...
            _, exit_requested = maybe_update_lr_or_exit()
            if exit_requested:
                exiting = True
                break
            ...
            advance = ema_kl <= cfg.kl_threshold and blend_progress < cfg.blend_steps
            if advance:
                blend_progress += 1
